...
def _artist_tracks_cards_fragment(con, user_id: int | None, artist_name: str, offset: int) -> str:
    ...
    cards_html = "".join(
        card(
            t["name"],
            u(f"/track/{quote(t['name'])}?artist={quote(artist_name)}"),
            note=str(t["cnt"]),
    ...
    return paginated_fragment(
        ...
        offset=offset,
        has_more=has_more,
        next_href=u(f"/artist/{quote(artist_name)}/tracks?offset={offset + ARTIST_TRACKS_BATCH}"),
    )
    ...
    if result:
        ...
        tracks_html = grid(
            "".join(
                card(
                    name,
                    u(f"/track/{quote(name)}?artist={quote(artist_name)}"),
                    note=str(count),
