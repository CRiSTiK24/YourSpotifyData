...
def ensure_monthly_play_count_tables(con: sqlite3.Connection) -> None:
    ...
    con.executescript(
        ...
        CREATE TABLE track_play_counts (
            ...
            album TEXT,
            PRIMARY KEY (user_id, name, singer, period)
        );
        CREATE INDEX idx_track_play_counts_period_count ON track_play_counts(user_id, period, play_count DESC);
        ...
        CREATE TABLE album_play_counts (
            ...
            period INTEGER NOT NULL,
            play_count INTEGER NOT NULL,
            PRIMARY KEY (user_id, album, singer, period)
        );
        CREATE INDEX idx_album_play_counts_period_count ON album_play_counts(user_id, period, play_count DESC);
        ...
        CREATE TABLE artist_play_counts (
            ...
            period INTEGER NOT NULL,
            play_count INTEGER NOT NULL,
            PRIMARY KEY (user_id, singer, period)
        );
        CREATE INDEX idx_artist_play_counts_period_count ON artist_play_counts(user_id, period, play_count DESC);

        INSERT INTO track_play_counts (user_id, name, singer, period, play_count, album)
        SELECT user_id, name, COALESCE(singer, ''), CAST(strftime('%Y%m', time) AS INTEGER), COUNT(*), MAX(album)
        FROM track_history WHERE name IS NOT NULL AND name != ''
        GROUP BY user_id, name, COALESCE(singer, ''), CAST(strftime('%Y%m', time) AS INTEGER);

        INSERT INTO track_play_counts (user_id, name, singer, period, play_count, album)
        SELECT user_id, name, COALESCE(singer, ''), 0, COUNT(*), MAX(album)
        FROM track_history WHERE name IS NOT NULL AND name != ''
        GROUP BY user_id, name, COALESCE(singer, '');

        INSERT INTO album_play_counts (user_id, album, singer, period, play_count)
        SELECT user_id, album, singer, CAST(strftime('%Y%m', time) AS INTEGER), COUNT(*)
        FROM track_history WHERE album IS NOT NULL AND album != '' AND singer IS NOT NULL AND singer != ''
        GROUP BY user_id, album, singer, CAST(strftime('%Y%m', time) AS INTEGER);

        INSERT INTO album_play_counts (user_id, album, singer, period, play_count)
        SELECT user_id, album, singer, 0, COUNT(*)
        FROM track_history WHERE album IS NOT NULL AND album != '' AND singer IS NOT NULL AND singer != ''
        GROUP BY user_id, album, singer;

        INSERT INTO artist_play_counts (user_id, singer, period, play_count)
        SELECT user_id, singer, CAST(strftime('%Y%m', time) AS INTEGER), COUNT(*)
        FROM track_history WHERE singer IS NOT NULL AND singer != ''
        GROUP BY user_id, singer, CAST(strftime('%Y%m', time) AS INTEGER);

        INSERT INTO artist_play_counts (user_id, singer, period, play_count)
        SELECT user_id, singer, 0, COUNT(*)
        FROM track_history WHERE singer IS NOT NULL AND singer != ''
        GROUP BY user_id, singer;

        CREATE TRIGGER track_play_counts_ai AFTER INSERT ON track_history
        WHEN new.name IS NOT NULL AND new.name != ''
        BEGIN
            INSERT INTO track_play_counts (user_id, name, singer, period, play_count, album)
            VALUES (new.user_id, new.name, COALESCE(new.singer, ''), CAST(strftime('%Y%m', new.time) AS INTEGER), 1, new.album)
            ON CONFLICT (user_id, name, singer, period) DO UPDATE SET play_count = play_count + 1, album = excluded.album;
            INSERT INTO track_play_counts (user_id, name, singer, period, play_count, album)
            VALUES (new.user_id, new.name, COALESCE(new.singer, ''), 0, 1, new.album)
            ON CONFLICT (user_id, name, singer, period) DO UPDATE SET play_count = play_count + 1, album = excluded.album;
        END;
        ...
        BEGIN
            UPDATE track_play_counts SET play_count = play_count - 1
                WHERE user_id = old.user_id AND name = old.name AND singer = COALESCE(old.singer, '')
            ...
            UPDATE track_play_counts SET play_count = play_count - 1
                WHERE user_id = old.user_id AND name = old.name AND singer = COALESCE(old.singer, '') AND period = 0;
            DELETE FROM track_play_counts
                WHERE user_id = old.user_id AND name = old.name AND singer = COALESCE(old.singer, '') AND period = 0 AND play_count <= 0;
        END;
        ...
        BEGIN
            INSERT INTO album_play_counts (user_id, album, singer, period, play_count)
            VALUES (new.user_id, new.album, new.singer, CAST(strftime('%Y%m', new.time) AS INTEGER), 1)
            ON CONFLICT (user_id, album, singer, period) DO UPDATE SET play_count = play_count + 1;
            INSERT INTO album_play_counts (user_id, album, singer, period, play_count)
            VALUES (new.user_id, new.album, new.singer, 0, 1)
            ON CONFLICT (user_id, album, singer, period) DO UPDATE SET play_count = play_count + 1;
        END;
        ...
        BEGIN
            UPDATE album_play_counts SET play_count = play_count - 1
                WHERE user_id = old.user_id AND album = old.album AND singer = old.singer
                  AND period = CAST(strftime('%Y%m', old.time) AS INTEGER);
            DELETE FROM album_play_counts
                WHERE user_id = old.user_id AND album = old.album AND singer = old.singer
                  AND period = CAST(strftime('%Y%m', old.time) AS INTEGER) AND play_count <= 0;
            UPDATE album_play_counts SET play_count = play_count - 1
                WHERE user_id = old.user_id AND album = old.album AND singer = old.singer AND period = 0;
            DELETE FROM album_play_counts
                WHERE user_id = old.user_id AND album = old.album AND singer = old.singer AND period = 0 AND play_count <= 0;
        END;
        ...
        BEGIN
            INSERT INTO artist_play_counts (user_id, singer, period, play_count)
            VALUES (new.user_id, new.singer, CAST(strftime('%Y%m', new.time) AS INTEGER), 1)
            ON CONFLICT (user_id, singer, period) DO UPDATE SET play_count = play_count + 1;
            INSERT INTO artist_play_counts (user_id, singer, period, play_count)
            VALUES (new.user_id, new.singer, 0, 1)
            ON CONFLICT (user_id, singer, period) DO UPDATE SET play_count = play_count + 1;
        END;
        ...
        BEGIN
            UPDATE artist_play_counts SET play_count = play_count - 1
                WHERE user_id = old.user_id AND singer = old.singer AND period = CAST(strftime('%Y%m', old.time) AS INTEGER);
            DELETE FROM artist_play_counts
                WHERE user_id = old.user_id AND singer = old.singer AND period = CAST(strftime('%Y%m', old.time) AS INTEGER) AND play_count <= 0;
            UPDATE artist_play_counts SET play_count = play_count - 1
                WHERE user_id = old.user_id AND singer = old.singer AND period = 0;
            DELETE FROM artist_play_counts
                WHERE user_id = old.user_id AND singer = old.singer AND period = 0 AND play_count <= 0;
        END;
        ...
        BEGIN
            UPDATE track_play_counts SET play_count = play_count - 1
                WHERE old.name IS NOT NULL AND old.name != ''
                  AND user_id = old.user_id AND name = old.name AND singer = COALESCE(old.singer, '')
                  AND period = CAST(strftime('%Y%m', old.time) AS INTEGER);
            DELETE FROM track_play_counts
                WHERE old.name IS NOT NULL AND old.name != ''
                  AND user_id = old.user_id AND name = old.name AND singer = COALESCE(old.singer, '')
            ...
            UPDATE track_play_counts SET play_count = play_count - 1
                WHERE old.name IS NOT NULL AND old.name != ''
                  AND user_id = old.user_id AND name = old.name AND singer = COALESCE(old.singer, '') AND period = 0;
            DELETE FROM track_play_counts
                WHERE old.name IS NOT NULL AND old.name != ''
                  AND user_id = old.user_id AND name = old.name AND singer = COALESCE(old.singer, '') AND period = 0 AND play_count <= 0;

            INSERT INTO track_play_counts (user_id, name, singer, period, play_count, album)
            SELECT new.user_id, new.name, COALESCE(new.singer, ''), CAST(strftime('%Y%m', new.time) AS INTEGER), 1, new.album
            WHERE new.name IS NOT NULL AND new.name != ''
            ON CONFLICT (user_id, name, singer, period) DO UPDATE SET play_count = play_count + 1, album = excluded.album;
            INSERT INTO track_play_counts (user_id, name, singer, period, play_count, album)
            SELECT new.user_id, new.name, COALESCE(new.singer, ''), 0, 1, new.album
            WHERE new.name IS NOT NULL AND new.name != ''
            ON CONFLICT (user_id, name, singer, period) DO UPDATE SET play_count = play_count + 1, album = excluded.album;
        END;
        ...
        BEGIN
            UPDATE album_play_counts SET play_count = play_count - 1
                WHERE old.album IS NOT NULL AND old.album != '' AND old.singer IS NOT NULL AND old.singer != ''
                  AND user_id = old.user_id AND album = old.album AND singer = old.singer
                  AND period = CAST(strftime('%Y%m', old.time) AS INTEGER);
            DELETE FROM album_play_counts
                WHERE old.album IS NOT NULL AND old.album != '' AND old.singer IS NOT NULL AND old.singer != ''
                  AND user_id = old.user_id AND album = old.album AND singer = old.singer
            ...
            UPDATE album_play_counts SET play_count = play_count - 1
                WHERE old.album IS NOT NULL AND old.album != '' AND old.singer IS NOT NULL AND old.singer != ''
                  AND user_id = old.user_id AND album = old.album AND singer = old.singer AND period = 0;
            DELETE FROM album_play_counts
                WHERE old.album IS NOT NULL AND old.album != '' AND old.singer IS NOT NULL AND old.singer != ''
                  AND user_id = old.user_id AND album = old.album AND singer = old.singer AND period = 0 AND play_count <= 0;

            INSERT INTO album_play_counts (user_id, album, singer, period, play_count)
            SELECT new.user_id, new.album, new.singer, CAST(strftime('%Y%m', new.time) AS INTEGER), 1
            WHERE new.album IS NOT NULL AND new.album != '' AND new.singer IS NOT NULL AND new.singer != ''
            ON CONFLICT (user_id, album, singer, period) DO UPDATE SET play_count = play_count + 1;
            INSERT INTO album_play_counts (user_id, album, singer, period, play_count)
            SELECT new.user_id, new.album, new.singer, 0, 1
            WHERE new.album IS NOT NULL AND new.album != '' AND new.singer IS NOT NULL AND new.singer != ''
            ON CONFLICT (user_id, album, singer, period) DO UPDATE SET play_count = play_count + 1;
        END;
        ...
        BEGIN
            UPDATE artist_play_counts SET play_count = play_count - 1
                WHERE old.singer IS NOT NULL AND old.singer != ''
                  AND user_id = old.user_id AND singer = old.singer AND period = CAST(strftime('%Y%m', old.time) AS INTEGER);
            DELETE FROM artist_play_counts
                WHERE old.singer IS NOT NULL AND old.singer != ''
                  AND user_id = old.user_id AND singer = old.singer AND period = CAST(strftime('%Y%m', old.time) AS INTEGER) AND play_count <= 0;
            UPDATE artist_play_counts SET play_count = play_count - 1
                WHERE old.singer IS NOT NULL AND old.singer != '' AND user_id = old.user_id AND singer = old.singer AND period = 0;
            DELETE FROM artist_play_counts
                WHERE old.singer IS NOT NULL AND old.singer != '' AND user_id = old.user_id AND singer = old.singer AND period = 0 AND play_count <= 0;

            INSERT INTO artist_play_counts (user_id, singer, period, play_count)
            SELECT new.user_id, new.singer, CAST(strftime('%Y%m', new.time) AS INTEGER), 1
            WHERE new.singer IS NOT NULL AND new.singer != ''
            ON CONFLICT (user_id, singer, period) DO UPDATE SET play_count = play_count + 1;
            INSERT INTO artist_play_counts (user_id, singer, period, play_count)
            SELECT new.user_id, new.singer, 0, 1
            WHERE new.singer IS NOT NULL AND new.singer != ''
            ON CONFLICT (user_id, singer, period) DO UPDATE SET play_count = play_count + 1;
        END;
        """
